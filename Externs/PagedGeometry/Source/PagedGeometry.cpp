...
namespace Forests {
    ...
    void GeometryPageManager::update(unsigned long deltaTime, Vector3& camPos, Vector3& camSpeed, bool& enableCache, GeometryPageManager* prevManager) {
        ...
        if (scrollBuffer) {
            ...
            if (x1 < 0) { shiftX = x1; }
            else if (x2 >= geomGridX - 1) { shiftX = x2 - (geomGridX - 1); }

            if (z1 < 0) { shiftZ = z1; }
            else if (z2 >= geomGridZ - 1) { shiftZ = z2 - (geomGridZ - 1); }
    ...
    void GeometryPageManager::reloadGeometryPages(const Vector3& center, Real radius) {
        ...
        if (x1 < 0) { x1 = 0; }
        else if (x1 > geomGridX - 1) { x1 = geomGridX - 1; }

        if (z1 < 0) { z1 = 0; }
        else if (z1 > geomGridZ - 1) { z1 = geomGridZ - 1; }
        ...
        if (x2 < 0) { x2 = 0; }
        else if (x2 > geomGridX - 1) { x2 = geomGridX - 1; }

        if (z2 < 0) { z2 = 0; }
        else if (z2 > geomGridZ - 1) { z2 = geomGridZ - 1; }
