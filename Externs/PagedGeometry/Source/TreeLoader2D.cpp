...
namespace Forests {
    ...
    TreeLoader2D::deleteTrees(const Ogre::Vector3& position, Ogre::Real radius, Entity* type) {
        ...
        if (minPageX < 0) { minPageX = 0; }
        else if (minPageX >= pageGridX) { minPageX = pageGridX - 1; }

        if (minPageZ < 0) { minPageZ = 0; }
        else if (minPageZ >= pageGridZ) { minPageZ = pageGridZ - 1; }

        if (maxPageX < 0) { maxPageX = 0; }
        else if (maxPageX >= pageGridX) { maxPageX = pageGridX - 1; }

        if (maxPageZ < 0) { maxPageZ = 0; }
        else if (maxPageZ >= pageGridZ) { maxPageZ = pageGridZ - 1; }
