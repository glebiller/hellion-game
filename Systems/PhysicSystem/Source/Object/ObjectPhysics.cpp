    ...
    if (pBody != NULL) {
        ...
        switch (pShape->getType()) {
            ...
            case hkcdShapeType::CONVEX_VERTICES:
                //case hkcdShapeType::PACKED_CONVEX_VERTICES:
        ...
        if (it->GetFlags() & Properties::Flags::Valid) {
            ...
            if (sName == sm_kapszCommonPropertyNames[ Property_Mass ]) {
                ...
                if (m_pBody != NULL) {
                    ...
                    switch (m_Quality) {
                        case 0:
                            hkQuality = HK_COLLIDABLE_QUALITY_DEBRIS;
                            break;

                        case 1:
                            hkQuality = HK_COLLIDABLE_QUALITY_MOVING;
                            break;

                        case 2:
                            hkQuality = HK_COLLIDABLE_QUALITY_BULLET;
                            break;

                        default:
                            hkQuality = HK_COLLIDABLE_QUALITY_CRITICAL;
                            break;
                    }
    ...
    if (m_pBody == NULL) {
        ...
        if (ChangeType & System::Changes::Graphics::AllMesh) {
            ...
            if (m_Type == Type_Box) {
                ...
                //
                const hkVector4* Vertices = reinterpret_cast<const hkVector4*>(pVertexBuffer + iPosition);
                hkStridedVertices StridedVerts(Vertices, VertexCount);
